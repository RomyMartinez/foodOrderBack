...
app.get('/meals', async (req, res) => {
  try {
    const meals = await fs.readFile(
      '../public/data/available-meals.json',
      'utf8'
...
app.post('/orders', async (req, res) => {
  ...
  const orders = await fs.readFile('../public/data/orders.json', 'utf8')
  const allOrders = JSON.parse(orders)
  allOrders.push(newOrder)
  await fs.writeFile('../public/data/orders.json', JSON.stringify(allOrders))
